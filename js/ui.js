const UI = {
  generarTarjetaPlato(item, rol, currentSelection = []) {
    ...
    if (rol === 'admin') {
       actionButtons = `
        <button data-action="toggle-pause" data-id="${escapeHTML(item.id)}" title="Retira el plato del Menú" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isPausado ? 'bg-primary-container text-sushi-white' : 'bg-surface-variant text-on-surface hover:bg-surface-bright'}">
        ...
        <button data-action="delete" data-id="${escapeHTML(item.id)}" title="Borrar Plato" class="w-10 h-10 rounded-full bg-surface-variant text-on-surface flex items-center justify-center hover:bg-primary-container/20 hover:text-primary transition-colors">
