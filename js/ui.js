const UI = {
  generarTarjetaPlato(item, rol, currentSelection = []) {
    ...
    // =====================================================================
    // 1. VISTA ADMINISTRATIVO (CLON PÍXEL POR PÍXEL DEL MODAL)
    // =====================================================================
    if (rol === 'admin') {
      const adminStyles = isPausado ? 'opacity-60 grayscale-[40%] border-primary/40' : 'border-primary-container/20';
      const imgTag = optimizedImgUrl 
        ? `<img src="${escapeHTML(optimizedImgUrl)}" alt="${escapeHTML(item.nombre)}" loading="lazy" class="absolute inset-0 w-full h-full object-cover transition-transform duration-300 hover:scale-105" />` 
        : `<span class="font-label-bold text-on-surface-variant z-0">Sin imagen</span>`;
      
      const promoTag = hasPromoText 
        ? `<div class="bg-primary text-sushi-white font-label-bold text-[10px] px-2 py-1 rounded shadow-lg transform -rotate-3 border border-sushi-white/20 w-fit text-center leading-tight">${escapeHTML(item.texto_promo)}</div>` 
        : '';
        
      const picanteTag = isPicante
      ...
      return `
        <div class="menu-card bg-[#121212] rounded-lg border overflow-hidden flex flex-col w-full h-full relative glow-hover transition-all duration-300 group ${adminStyles}">
          <div class="menu-card-img-wrapper h-48 relative bg-surface-container-highest flex items-center justify-center overflow-hidden shrink-0">
            ${imgTag}
          ...
          <div class="p-4 flex flex-col flex-grow bg-[#121212]">
            ...
            <p data-expandable="true" class="font-body-md text-on-surface-variant text-sm flex-grow line-clamp-3 cursor-pointer select-none transition-all duration-200">${escapeHTML(item.descripcion || '')}</p>
            
            <div class="mt-auto pt-2 flex gap-2 empty:hidden">${picanteTag}</div>
            
            <div class="flex justify-end gap-2 pt-3 mt-4 border-t border-outline-variant/20">
              <button data-action="toggle-pause" data-id="${escapeHTML(item.id)}" title="Retira el plato del Menú" class="w-9 h-9 rounded-full flex items-center justify-center transition-colors ${isPausado ? 'bg-primary text-sushi-white' : 'bg-surface-variant text-on-surface hover:bg-surface-bright'}">
                <span class="material-symbols-outlined text-[18px] pointer-events-none">pause_circle</span>
              </button>
              <button data-action="edit" data-id="${escapeHTML(item.id)}" title="Editar Plato" class="w-9 h-9 rounded-full bg-surface-variant text-on-surface flex items-center justify-center hover:bg-primary/20 hover:text-primary transition-colors">
                <span class="material-symbols-outlined text-[18px] pointer-events-none">edit</span>
              </button>
              <button data-action="delete" data-id="${escapeHTML(item.id)}" title="Borrar Plato" class="w-9 h-9 rounded-full bg-surface-variant text-on-surface flex items-center justify-center hover:bg-error/20 hover:text-error transition-colors">
    ...
    // =====================================================================
    // 2. VISTA CLIENTE (INTACTA - TERRENO PROHIBIDO)
    // =====================================================================
    const imgTagClient = optimizedImgUrl 
    ...
    const promoTagClient = hasPromoText 
    ...
    return `
      <div class="menu-card bg-surface-container-low rounded-lg border border-primary/20 overflow-hidden flex flex-col sm:flex-row glow-hover transition-all duration-300 relative group">
        ...
        <div class="p-5 w-full sm:w-3/5 flex flex-col flex-grow">
          ...
          <div class="flex justify-between items-center gap-2 pt-4 border-t border-outline-variant/20 mt-auto">
